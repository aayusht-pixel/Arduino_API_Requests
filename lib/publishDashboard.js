...
var fs = require('fs'); // required by the File-System API

// get the access token, from a separate file for security
var tokenGetter = require('./getToken');

// make GET request on aayush dashboard to display its contents and publish it to a JSON file
async function myHttpRequest(myUrl, myMethod, myToken) {
    ...
    xhr.onload = () => {
        ...
        // write parsed data to JSON file, with formatting
        fs.writeFile('./myDashboard.json', JSON.stringify(data, null, 4), err => {
            if (err) {
                console.error(err);
            }
