...
var payloadPublish = JSON.stringify(require('./publishThingPropertyPayload2.json')); // include dashboard upadate in a JSON file

// get the access token, from a separate file for security
var tokenGetter = require('./getToken');

// make PUT request on aayush_thing thing to publish on a property
async function myHttpRequest(myUrl, myMethod, myToken) {
    ...
    xhr.send(payloadPublish); // send Http Request
}
...
async function run() {
    ...
    // URL of target thing, property and /publish trailer, i.e., aayush_thing for aayush_int_value variable
    var url = 'https://api2.arduino.cc/iot/v2/things/7ea1ea47-9ca5-4805-94eb-8185d489ac1a/properties/722dc691-60e2-4615-bcc9-97c1ec0f0085/publish'; 
